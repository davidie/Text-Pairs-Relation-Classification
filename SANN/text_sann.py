...
class TextSANN(object):
    ...
    def __init__(
        ...
        self.M_flat_front = tf.reshape(self.M_front, [-1, attention_hops_size * lstm_hidden_size * 2])
        self.M_flat_behind = tf.reshape(self.M_behind, [-1, attention_hops_size * lstm_hidden_size * 2])

        # shape of `M_flat_combine`: [batch_size, attention_hops_size * lstm_hidden_size * 2 * 2]
        self.M_flat_combine = tf.concat([self.M_flat_front, self.M_flat_behind], 1)
